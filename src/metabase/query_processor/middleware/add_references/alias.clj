(ns metabase.query-processor.middleware.add-references.alias
...
(defmulti prefix-field-alias
  ...
  {:arglists '([driver prefix field]), :added "0.42.0"}
  driver/dispatch-on-initialized-driver
  :hierarchy #'driver/hierarchy)

(defmethod prefix-field-alias :driver
...
(defmethod clause-alias [:driver :expression]
  [_driver [_ expression-name]]
  expression-name)

(defmethod clause-alias [:driver :field]
  [driver [_ name-or-id :as field-clause]]
  (if (string? name-or-id)
    name-or-id
    (unambiguous-field-alias driver field-clause)))

(defmethod clause-alias [:driver :aggregation-options]
  [_driver [_ _ {ag-name :name}]]
  ag-name)
